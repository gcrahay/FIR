...
def details(request, incident_id):
	...
	return render(request, "events/detail-all.html", {	"event": i,
														"comment_form": form,
...
def new_event(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if form.cleaned_data['is_major'] == False:
...
def search(request):
	...
	if ('q' in request.GET) and request.GET['q'].strip():
		...
		if request.is_ajax():
			...
			if order_by == "category": order_by = 'category__name'
			if order_by == 'detection': order_by = 'detection__name'
			if order_by == 'actor': order_by = 'actor__name'
...
def sandbox(request, divisor='all'):
	...
	return render(request, 'stats/sandbox.html', {	'start': fr,
													'end': to,
													'form': form,
...
def stats_attributes(request):
	...
	return render(request, 'stats/attributes.html', { 'start': fr,
													  'end': to,
													  'form': form,
													  'categories': categories,
													  'attributes': attributes })
